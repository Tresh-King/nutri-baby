...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method === 'GET') {
        const record = await prisma.diaperRecord.findUnique({ where: { id: recordId } });
        if (!record) return res.status(404).json({ message: 'Not Found' });
        const response = JSON.parse(JSON.stringify(record, (key, value) => typeof value === 'bigint' ? value.toString() : value));
        return res.status(200).json(response);
    } else if (req.method === 'PUT') {
        const { time, type, poopColor, poopTexture, note } = req.body;
        try {
            const updated = await prisma.diaperRecord.update({
                where: { id: recordId },
                data: { time: time ? new Date(time) : undefined, type, poopColor, poopTexture, note }
            });
        ...
        try {
            await prisma.diaperRecord.delete({ where: { id: recordId } });
